...
def cli(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        console.print("  or [bold cyan]python3 aarchai.py --help[/] for all commands.\n")
...
@click.option("--status",
              type=click.Choice(["confirmed", "false_positive", "accepted_risk"]),
              required=True)
def triage(scan_id, finding_id, status):
    """Triage a finding."""
    from core.db import get_session, Finding
    session = get_session()
    f = session.query(Finding).filter(
    ...
    session.commit()
    console.print(f"[green]Finding #{finding_id} -> {status}[/]")
    session.close()
...
def suppress(tool, finding_type, title, reason):
    ...
    session.add(SuppressionRule(
        tool=tool, finding_type=finding_type,
        title_pattern=title, reason=reason
    ))
    session.commit()
    console.print("[green]Suppression rule added[/]")
    session.close()
...
@click.option("--framework",
              type=click.Choice(["owasp", "pci", "iso27001", "nist"]),
              default="owasp")
@click.option("--output-dir",  "-o", default="./reports")
def compliance(scan_id, framework, output_dir):
    ...
    target = session.get(Target, sc.target_id) if sc else None
